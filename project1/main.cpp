...
int main() {
    sf::RenderWindow window(sf::VideoMode({ 1000, 700 }), "Project1");
    ...
    while (window.isOpen()) {
        ...
        while (const std::optional event = window.pollEvent()) {
            ...
            if (const auto* mBtn = event->getIf<sf::Event::MouseButtonPressed>()) {
                ...
                if (game.state == GameState::MENU) {
                    ...
                    // 2. 건너뛰기 버튼 처리
                    if (game.skipRewardBtn.shape.getGlobalBounds().contains(mPos)) {
                        game.startNextFloor(); // 카드 추가 없이 바로 다음 층으로
                    }
                }
                    ...
                    if (!game.isRemovingCard) {
                        ...
                        for (int i = 0; i < (int)game.shopRelics.size(); i++) {
                            if (game.shopRelics[i].shape.getGlobalBounds().contains(mPos)) {
                                ...
                                // 바로 startNextFloor를 하지 않고, 삭제 모드를 켭니다.
                                game.isRemovingCard = true;
                                break;
                            }
                        }
                    }
                        ...
                        // 카드 삭제
                        for (int i = 0; i < (int)game.masterDeck.size(); i++) {
                            if (game.masterDeck[i].shape.getGlobalBounds().contains(mPos)) {
                                game.removeCardFromMasterDeck(i); // 이 함수 안에서 startNextFloor() 호출됨
                                break;
                            }
                        }

                        // 삭제 건너뛰기 버튼 클릭 시
                        if (game.skipRemoveBtn.shape.getGlobalBounds().contains(mPos)) {
                            game.isRemovingCard = false;
                            game.startNextFloor();
                        }
                    }
                }
                ...
                // 2. 덱 화면에서 돌아가기 (기존 backBtn 활용)
                    ...
                    if (!game.isRemovingCard) {
                        ...
                        // 2. 카드 삭제 단계
                        // 마스터 덱의 카드들 중 클릭한 것이 있는지 확인 (renderDeck 로직과 위치 동일해야 함)
                        for (int i = 0; i < (int)game.masterDeck.size(); i++) {
                            if (game.masterDeck[i].shape.getGlobalBounds().contains(mPos)) {
                                game.removeCardFromMasterDeck(i);
                                break;
                            }
                        }
                        // 삭제 건너뛰기 버튼
        ...
        if (game.state == GameState::MENU) {
            sf::Text title(game.font, "Project1", 60);
            ...
            floorText.setPosition({ 650, 10 }); // 몬스터 HP보다 위쪽인 10에 배치
            ...
            for (auto& c : game.hand) {
                window.draw(c.shape);
                cardText.setString(c.name + L"\nCost: " + std::to_wstring(c.cost) + L"\nVal: " + std::to_wstring(c.value));
            ...
            window.draw(game.skipRewardBtn.shape);
            window.draw(game.skipRewardBtn.text);
        }
        else if (game.state == GameState::RANKING) {
            ...
            if (!game.isRemovingCard) {
                // --- 상점 화면 ---
                sf::Text shopT(game.font, L"둘중 하나를 선택하세요", 45);
                shopT.setPosition({ 250.f, 100.f });
                ...
                for (auto& r : game.shopRelics) {
                    ...
                    sf::Text rText(game.font, r.name + L"\n" + r.description, 18);
                    rText.setPosition(r.shape.getPosition() + sf::Vector2f(0.f, 110.f));
                ...
                sf::Text removeT(game.font, L"제거할 카드를 선택하세요", 40);
                removeT.setPosition({ 230.f, 50.f });
                removeT.setFillColor(sf::Color::Red);
                window.draw(removeT);

                // 덱의 카드들을 격자 형태로 배치하여 출력
                for (int i = 0; i < (int)game.masterDeck.size(); i++) {
                    float x = 80.f + (i % 6) * 150.f;  // 가로 6장씩 배치
                    float y = 150.f + (i / 6) * 200.f; // 세로 간격 200

                    // 카드 본체 위치 설정 및 그리기
                    game.masterDeck[i].shape.setPosition({ x, y });
                    window.draw(game.masterDeck[i].shape);

                    // 카드 정보 텍스트 위치 설정 및 그리기
                    cardText.setString(game.masterDeck[i].name + L"\nCost: " + std::to_wstring(game.masterDeck[i].cost));
                ...
                // 삭제하지 않고 넘어가는 버튼
                window.draw(game.skipRemoveBtn.shape);
                window.draw(game.skipRemoveBtn.text);
            }
        }
