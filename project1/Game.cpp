...
Game::Game() : rng(std::random_device{}()), startBtn(font), rankBtn(font), backBtn(font),
               endTurnBtn(font), viewDeckBtn(font), skipRemoveBtn(font), skipRewardBtn(font) {}
...
void Game::init() {
    ...
    endTurnBtn.text.setString(L"턴 종료");
    endTurnBtn.text.setCharacterSize(22);
    endTurnBtn.text.setPosition({ 840.f, 595.f });

    rewardPool.push_back(createCardData(L"강타", CardType::ATTACK, 2, 25));
    rewardPool.push_back(createCardData(L"철벽", CardType::DEFENSE, 2, 20));
    rewardPool.push_back(createCardData(L"대회복", CardType::HEAL, 2, 20));
    rewardPool.push_back(createCardData(L"Double Slash", CardType::ATTACK, 1, 12));
    rewardPool.push_back(createCardData(L"막기", CardType::DEFENSE, 1, 10));
    rewardPool.push_back(createCardData(L"힐", CardType::HEAL, 1, 8));
    rewardPool.push_back(createCardData(L"분쇄", CardType::ATTACK, 3, 40));
    ...
    viewDeckBtn.text.setString(L"덱 보기");
    viewDeckBtn.text.setCharacterSize(20);
    viewDeckBtn.text.setPosition({ 840.f, 492.f });

    skipRemoveBtn.shape.setSize({ 220.f, 60.f });
    skipRemoveBtn.shape.setPosition({ 400.f, 600.f }); // 화면 하단 중앙
    skipRemoveBtn.shape.setFillColor(sf::Color(231, 76, 60));
    skipRemoveBtn.text.setString(L"SKIP REMOVAL");
    skipRemoveBtn.text.setCharacterSize(22);
    skipRemoveBtn.text.setPosition({ 425.f, 615.f });

    skipRewardBtn.shape.setSize({ 200.f, 60.f });
    skipRewardBtn.shape.setPosition({ 400.f, 580.f });
    skipRewardBtn.shape.setFillColor(sf::Color(127, 140, 141)); // 회색
    skipRewardBtn.text.setString(L"SKIP REWARD");
    skipRewardBtn.text.setCharacterSize(22);
    skipRewardBtn.text.setPosition({ 435.f, 595.f });

    // 에너지 증가 카드
    rewardPool.push_back(createCardData(L"에너지증가", CardType::UTILITY, 0, 2));
    // 0코스트 (에너지 증가용)

    // 드로우 카드
    rewardPool.push_back(createCardData(L"카드뽑기", CardType::UTILITY, 1, 3));
    // 1코스트 (드로우용)
...
Card Game::createCardData(std::wstring name, CardType type, int cost, int value) {
...
void Game::playCard(int i) {
    ...
    if (energy >= c.cost) {
        ...
        else if (c.type == CardType::UTILITY) {
            if (c.name == L"에너지증가") {
                // 에너지 회복 (최대 에너지 제한은 기획에 따라 조절)
                energy += c.value;
            }
            else if (c.name == L"카드뽑기") {
                // 카드 드로우 (최대 5장 제한)
                for (int d = 0; d < c.value; d++) {
                    if (hand.size() < 5) { // 손패가 5장 미만일 때만 드로우
                        drawCards(1);
                    }
                }
            }
...
void Game::startNextFloor() {
    isRemovingCard = false; // 새 층 시작 시 삭제 모드 확실히 종료
    // 게임 시작 시 덱 초기화 (0층에서 1층 갈 때)
    if (floor == 0) {
        masterDeck.clear();
        for (int i = 0; i < 5; i++) masterDeck.push_back(createCardData(L"타격", CardType::ATTACK, 1, 8));
        for (int i = 0; i < 5; i++) masterDeck.push_back(createCardData(L"수비", CardType::DEFENSE, 1, 6));
    }
    ...
    if (floor % 10 == 0) {
        isBossFloor = true;
        maxMonsterHP = 100 + (floor * 10);
        monsterAttack = (floor);
    }
    else {
        isBossFloor = false;
        maxMonsterHP = 30 + (floor * 5);
        monsterAttack = 6 + (floor / 2);
    }
...
void Game::renderDeck(sf::RenderWindow& window, sf::Text& cardText) {
    ...
    for (int i = 0; i < (int)masterDeck.size(); i++) {
        ...
        cardText.setString(masterDeck[i].name + L"\nCost: " + std::to_wstring(masterDeck[i].cost));
...
void Game::removeCardFromMasterDeck(int index) {
    if (index >= 0 && index < (int)masterDeck.size()) {
        masterDeck.erase(masterDeck.begin() + index);
    }
    isRemovingCard = false; // 삭제 완료 후 모드 해제
    startNextFloor();       // 다음 층으로 이동
}